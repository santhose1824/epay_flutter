...
class GPPinVerifyScreenState extends State<GPPinVerifyScreen> {
  ...
  style: ButtonStyle(
    backgroundColor: MaterialStateProperty.all<Color>(Colors.white.withOpacity(0)), // Set to transparent for no background
    overlayColor: MaterialStateProperty.resolveWith<Color>(
      (states) {
        if (states.contains(MaterialState.pressed)) {
          return Colors.white.withOpacity(0.2); // Adjust opacity as needed
        }
    ...
    shape: MaterialStateProperty.all<OutlinedBorder>(
      RoundedRectangleBorder(
        ...
        side: BorderSide(color: Colors.white.withOpacity(0.4), width: 1), // Border color and width
      ),
    ),
  ),
  child: Text("Reset passcode", textAlign: TextAlign.center),
),
